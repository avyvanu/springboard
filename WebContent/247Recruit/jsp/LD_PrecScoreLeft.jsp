...
<%
	...
	String query="select Bpo_Cand_Id,First_Name,Middle_Name,Last_Sir_Name from bpo_candidatesreg_details a left join precis_test_attempt_master b  on a.autoid = b.USER_ID left join candidate_precis_writing_score c  on a.autoid = c.cand_id where b.USER_ID is Not null and b.End_Date is Not null and ((c.cand_id is null and a.cand_Status=4) or (c.cand_id is not null and a.cand_Status = 6 and a.MarkLangReScore = 1)) and a.Applying_For=2 and a.Location='"+User_Center_Id+"' and a.AutoId not in  (Select Cand_Id from candidate_precis_writing_score where recruiter_id in ('"+USER_ID+"'))"; 
	System.out.println("PrecisWRitingScore page::::::"+query);
...
function CheckValidation()
    ...
    function ChangeColor(tableRow, canId)
    {
	    ...
	    parent.right.location.href= "<%=recruitPath%>jsp/LD_PrecScoreRight.jsp?candId="+canId;
  	}
