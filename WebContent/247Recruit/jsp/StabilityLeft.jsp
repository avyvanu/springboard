...
String query="select AutoID,Bpo_Cand_Id,First_Name,Middle_Name,Last_Sir_Name from bpo_candidatesreg_details where cand_Status=4 and Applying_For=1 and Location='"+User_Center_Id+"'"; 
...
<html>
  <head>
    ...
    function ChangeColor(tableRow, canId)
    {
	    ...
	    parent.right.location.href= "<%=recruitPath%>jsp/StabilityRight.jsp?candId="+canId;
  	}
    ...
    <table width="420" rules="all" style="border:1px solid black;background-color:#F2F2F2;margin:0;font-family: Arial,Helvetica,sans-serif; font-size: 13px;">
					...
					<%
						...
						while(rs.next())
						{
							...
							String queryScore="select Total_score from candidate_voice_score where Cand_id=" + autoID +"";									
							Statement stmtScore=connection.createStatement();
							ResultSet rsScore=stmtScore.executeQuery(queryScore);
