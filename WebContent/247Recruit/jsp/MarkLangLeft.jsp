...
String query="select Bpo_Cand_Id,First_Name,Middle_Name,Last_Sir_Name from bpo_candidatesreg_details a left join language_test_attempt_master b  on a.autoid = b.Cand_Id left join candidate_language_testscore c on a.autoid=c.cand_id where b.Cand_Id  is Not null and b.End_Date is Not null  and  a.MarkLangReScore = 0 and (c.cand_id is not null and (a.cand_Status=4 or (a.cand_Status = 6 and a.Init_Result not in (1,3)))) and a.Applying_For=2 and a.Location='"+User_Center_Id+"'"; 
System.out.println("Language Test Score Query::::::"+query);
...
<html>
  <head>
    ...
    function ChangeColor(tableRow, canId)
    {
	    ...
	    parent.right.location.href= "<%=recruitPath%>jsp/MarkLangRight.jsp?candId="+canId;
  	}
