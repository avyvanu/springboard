 ...
 if(USER_Name == null)
{
 ...
 System.out.println("USER_Name in language script print page::::"+USER_Name);
       ...
       <tr>
           <td class="allBackColor" align="left" style="background-color: #D8D8D8;"><b>Passage Summary</b></td>
              <%
                ...
                Statement stments=null;
                ResultSet rssss=null;
                String secquery="";
                if(Part.equalsIgnoreCase("Partb"))
                {
                secquery = "select PartB_A  from language_test_attempt_master where Cand_Id="+id+"";
                System.out.println("query "+secquery);
				stments = con.createStatement();
				rssss = stments.executeQuery(secquery);
				if(rssss.next())
				{
				typedpara=(rssss.getString("PartB_A")).trim();
				System.out.println("typedpara "+typedpara);
				}
				}
				if(Part.equalsIgnoreCase("Partc"))
                {
                secquery = "select PartC_A  from language_test_attempt_master where Cand_Id="+id+"";
                System.out.println("query "+secquery);
				stments = con.createStatement();
				rssss = stments.executeQuery(secquery);
				if(rssss.next())
				{
				typedpara=(rssss.getString("PartC_A")).trim();
				System.out.println("typedpara "+typedpara);
				}
				}
			  %>
