...
System.out.println("USER_ID in LD script home page::::"+USER_ID);
...
System.out.println("User_Center_Id::::::Left LD script"+User_Center_Id);
String query="select Bpo_Cand_Id,First_Name,Middle_Name,Last_Sir_Name from bpo_candidatesreg_details a left join candidate_chat_score b on a.autoid=b.cand_id where ((b.cand_id is null and a.cand_Status=4) or (b.cand_id is not null and a.cand_Status = 6 and a.MarkChatReScore = 1)) and a.Applying_For=2 and a.Location='"+User_Center_Id+"' and a.AutoId  not in (Select Cand_Id from candidate_chat_score where recruiter_id in ('"+USER_ID+"'))"; 
System.out.println("query:::LD_CHATScriptLeft:::"+query);
...
function CheckValidation()
    ...
    function ChangeColor(tableRow, canId)
    {
	    ...
	    parent.right.location.href= "<%=recruitPath%>jsp/LD_ChatScriptRight.jsp?candId="+canId;
  	}
