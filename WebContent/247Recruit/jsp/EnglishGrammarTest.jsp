...
<%
	...
	try
	{
		String time_duration = "SELECT Time_limit_min,Time_limit_sec FROM test_master WHERE Test_id = 3";
...
<script LANGUAGE="JavaScript">  
		...
		function redo() {
	 	...
	 	document.engTestPage.disp.value = dis(mins,secs); // setup additional displays here.
	 	if((mins == 0) && (secs == 0)) 
	 	{
...
function validateText12()
	{	
		parent.parent.related.document.getElementById("test_flag").value="3";
...
<table border="0" width="100%" style="padding-top: 5px;">
     <tr>   
	    <td align="left" style="padding-left:10px;">
	        ...
	        <font style="color:#000000; font-weight:bold; font-size:12px; font-family:Arial; ">English Grammar Test</font>		
		</td>
...
<form name="engTestPage" id="engTestPage" action="<%=request.getContextPath()%>/EnglishGrammerTest" method="post">
   ...
   <input type="hidden" name="methodName" id="methodName" value="storeRes"/>
    <div class="forms" >
     ...
     <%
		...
		try
		{
            String compQsnQry = "SELECT * FROM bpo_eng_comp_master where Is_Alive = 1 AND is_deleted = 0 ORDER BY RAND() LIMIT 1";
			System.out.println("bpo_eng_comp_master   compQsnQry : "+compQsnQry);
			...
			if(adrRs.next())
			{
				...
				System.out.println("compQsnId : "+compQsnId+" compQsnPass : "+compQsnPass);
			}
		}
		catch (Exception e) 
		{
     ...
     %> 
      <tr>
        <th colspan="4" class="taL"><div style="float:left;">English Grammar Assessment </div><div style="float:right;">Time Remaining <input type="text" name="disp" size="3" class="timer" id="disp"/>  </div></th>
      </tr>
       <tr>
     ...
     <tr>
      <td style="padding-left:23px;"><div style="float: left;">
     <input type="hidden" name="target" value="EnglishGrammerTest"/>
	...
	<textarea class="quest_output" style="height: 311px; width: 329px;"  readonly="readonly" oncut="return false" oncopy="return false" onpaste="return false"><%= compQsnPass %></textarea>
      	
       </div>
        <div style="margin-left: 2px; width: 330px;" class="quest_output2">
        
         <% try
			{
				String compQsnQry12 = "SELECT * FROM bpo_eng_comp_question_master where READING_COMP_MASTER_ID="+compQsnId;
				System.out.println("bpo_eng_comp_question_master : "+compQsnQry12);
				...
				while(adrRs12.next())
				{
					...
					System.out.println("count is : "+count+" qsnID : "+qsnID);
					%>
      					<p><b><%=qsnID%>.&nbsp;&nbsp;<%= adrRs12.getString("PASSAGE_QUESTION")%></b></p>
      				<%	
				  		String compQsnQry123 = "SELECT * FROM bpo_eng_comp_question_answer_master where READING_COMP_MASTER_ID="+compQsnId+" and READING_COMP_QUESTION_MASTER_ID="+qsnID;
      		...
      		count++;
      		 ...
      		 %>
      		 <input type="hidden" name="countQuesNo" value="<%=count%>" />
      		 <%
      		 }
			catch (Exception e) 
			{
