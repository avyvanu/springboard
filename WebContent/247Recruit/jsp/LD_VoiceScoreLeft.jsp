...
<%
	...
	String query="select Bpo_Cand_Id,First_Name,Middle_Name,Last_Sir_Name from bpo_candidatesreg_details a left join stability_chk_master b on a.AutoID = b.Candidate_ID left join candidate_voice_score c on a.autoid = c.cand_id where a.Applying_For=1 and a.Location='"+User_Center_Id+"' AND ((c.cand_id is null and a.cand_Status=5 and b.result_status=1) or (c.cand_id is not null and a.cand_Status = 6 and a.MarkVoiceReScore = 1)) and a.AutoId not in (Select Cand_Id from candidate_voice_score where recruiter_id in ('"+USER_ID+"'))"; 
	System.out.println("LD_Voice Query  Details ::::"+query);
...
function CheckValidation()
    ...
    function ChangeColor(tableRow, canId)
    {
	    ...
	    parent.right.location.href= "<%=recruitPath%>jsp/LD_VoiceScoreRight.jsp?candId="+canId;
  	}
   ...
   <table width="420">
    ...
    <table width="420" rules="all" style="border:1px solid black;background-color:#F2F2F2;margin:0;font-family: Arial,Helvetica,sans-serif; font-size: 13px;">
					...
					<%
						...
						while(rs.next())
						{%>
							<tr id="<%=i%>" onclick="javascript:ChangeColor(this,'<%=rs.getString("Bpo_Cand_Id")%>');" onmouseover="style.fontWeight = 'bold'" onmouseout="style.fontWeight = 'normal'" style="cursor: pointer;">
								<td>
