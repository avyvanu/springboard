...
<%
	...
	try
	{
	     ...
	     System.out.println("USER_AUTOID in Admin home page::::"+USER_AUTOID);
	     User_Type_Id = (String)session.getAttribute("User_Type_Id");
	     if(User_Type_Id == null)
		  	{
		  	  response.sendRedirect(request.getContextPath() + "/247Recruit/jsp/Login.jsp");
		  	}
		 System.out.println("User_Type_Id in Admin home page::::"+User_Type_Id);
	     USER_Name = ((String)session.getAttribute("USER_Name")).trim();
	     if(USER_Name==null)
		  	{
		  	 response.sendRedirect(request.getContextPath() + "/247Recruit/jsp/Login.jsp");
		  	}
		 System.out.println("USER_Name in Admin home page::::"+USER_Name);
	    ...
	    <tr>
	        <td align="left" style="padding-left:10px;">
	            <font style="color:#7E7E7E; font-weight:bold; font-size:12px; font-family:Arial; "><a href="<%=recruitPath%>jsp/Home_Page.jsp">Home</a> > </font>
	             <font style="color:#000000; font-weight:bold; font-size:12px; font-family:Arial; ">Administration</font> 
	        </td>
...
<!--  class="thHeaderImage" -->
    ...
    <table width="733" border="0" cellpadding="0" cellspacing="0" align="center">
     	  <%
			...
			String task_URL = "";
			String qryforAdminTasks = "";
		try
		{
		 	if(IsSAdmin.equalsIgnoreCase("Yes"))
		 	{
		 		qryforAdminTasks = "select tm.task_id,tm.task_description,tm.task_url from task_master tm where pareent_task_id = 12";
		 	}
		 	else
		 	{
		 		qryforAdminTasks = "SELECT tm.Task_Id,tm.task_description,tm.task_url FROM User_Type_Task_Mapping uttm,task_master tm WHERE uttm.task_id = tm.task_id and tm.Is_Active = 1 and uttm.User_Type_Id = "+User_Type_Id+" and tm.Pareent_Task_Id = 12  AND uttm.UTT_Mapping_Id IN (SELECT UTT_Mapping_Id FROM User_UTT_Mapping WHERE User_Id = "+USER_AUTOID+")";
		 	}
		 	System.out.println("qryforAdminTasks::::"+qryforAdminTasks);
		 	con=ConnectDatabase.getConnection();
		 	stmtN=con.createStatement();
		    rsN=stmtN.executeQuery(qryforAdminTasks);
		    int i = 1 ;
		     while(rsN.next())
		      {
		    	task_id=rsN.getString("tm.Task_id");
		    	System.out.println("task_id in Admin Home Page::::"+task_id );
		    	taskDescription = rsN.getString("tm.task_description");
		    	System.out.println("taskDescription in Admin Home Page::::"+taskDescription );
		    	task_URL =  rsN.getString("tm.task_url");
		    	System.out.println("task_URL in Admin Home Page::::"+task_URL );
