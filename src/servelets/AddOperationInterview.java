...
public class AddOperationInterview extends HttpServlet 
{
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		throws ServletException, IOException 
		{
			...
			try
			{
				...
				pstmt = con.prepareStatement("UPDATE operations_interview_part_master SET IsAlive = ? WHERE Program_Id = ? AND IsAlive = 1");
				pstmt.setInt(1, 0);
				...
				for(int i = 1; i <= total_Parts; i++)
				{
					...
					pstmt1 = con.prepareStatement("INSERT INTO operations_interview_part_master (Program_Id, Part_Description, IsAlive, Order_No, Updated_By, Updated_On) VALUES(?, ?, ?, ?, ?, NOW())");
					pstmt1.setInt(1, programId);
					pstmt1.setString(2, partVal );
					pstmt1.setInt(3, 1);
					pstmt1.setInt(4, i);
					pstmt1.setString(5, USER_ID);
					insertPartMstr = pstmt1.executeUpdate();
					
					String query = "SELECT max(Part_id) as Part_id FROM operations_interview_part_master WHERE IsAlive = 1 AND Program_Id = '"+programId+"'";
					...
					for(int j = 1; j <= total_ques; j++ )
					{
						...
						pstmt2 = con.prepareStatement("INSERT INTO operations_interview_question_master (Part_id, Question, IsAlive, Order_No) VALUES(?, ?, ?, ?)");
				...
				if(updateQuery != 0)
				{
					if(insertPartMstr != 0)
					{
						if(insertQnsMstr != 0)
						{
							...
							response.sendRedirect("247Recruit/jsp/Admin_Manage_Operations_Qns.jsp");
						}
					}
				}
				else
