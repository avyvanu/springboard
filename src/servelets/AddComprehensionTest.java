...
public class AddComprehensionTest extends HttpServlet 
{
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		System.out.println("--- Inside the Add Comprehension Test ---");
		...
		try
		{
			...
			addCompMasterInsert = connection.prepareStatement("insert into bpo_reading_comp_master(Test_Name,Test_Header,PASSAGE,Is_Alive,Updated_By,Updated_On) values(?,?,?,?,?,now())");
			...
			System.out.println("Inserted Successfully into bpo_reading_comp_master::::");
			
			String qryForCompId  = "SELECT MAX(reading_comp_master_id) as compId FROM bpo_reading_comp_master";
			System.out.println("qryForCompId:::"+qryForCompId);
	        Statement stmtCompId=connection.createStatement();
	        ResultSet rsCompId=stmtCompId.executeQuery(qryForCompId);
	        if(rsCompId.next())
	        {
			...
			for(int i = 1; i < totalQuestions ; i++)
			{
				...
				addCompQnsInsert = connection.prepareStatement("insert into bpo_reading_comp_question_master(READING_COMP_QUESTION_MASTER_ID,READING_COMP_MASTER_ID,PASSAGE_QUESTION) values(?,?,?)");
				...
				System.out.println("Inserted Successfully into bpo_reading_comp_question_master::::");
				...
				for(int j = 1; j <= totalQnsOptions ; j++)
				{
					...
					addCompOptionsInsert = connection.prepareStatement("insert into bpo_reading_comp_question_answer_master(READING_COMP_QUESTION_ANSWER_MASTER_ID,READING_COMP_MASTER_ID,READING_COMP_QUESTION_MASTER_ID,PASSAGE_QUESTION_ANSWER,correct_answer) values(?,?,?,?,?)");
					...
					System.out.println("Inserted Successfully into bpo_reading_comp_question_answer_master::::");
				}
			}
		}
		catch(Exception e)
		...
		System.out.println(" Admin Added successfully questions in comprehension test ");
		...
		System.out.println(" After Forwarding the request add comprehension test");
	}
}
